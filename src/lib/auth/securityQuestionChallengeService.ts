...
class SecurityQuestionChallengeService {
  ...
  async getChallengeById(challengeId: string): Promise<SecurityQuestionChallengeDocument | null> {
    const collection = await getChallengeCollection();
    return collection.findOne<SecurityQuestionChallengeDocument>({ id: challengeId });
  }
