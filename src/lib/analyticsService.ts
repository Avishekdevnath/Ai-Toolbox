...
export class AnalyticsService {
  ...
  static async getSystemAnalytics() {
    try {
      ...
      const [totalUsage, uniqueUsers, popularTools, recentActivity] = await Promise.all([
        ToolUsage.countDocuments(),
        ToolUsage.distinct('userId').then(users => users.length),
        this.getPopularTools(5),
        ToolUsage.find().sort({ timestamp: -1 }).limit(10).lean()
      ]);
