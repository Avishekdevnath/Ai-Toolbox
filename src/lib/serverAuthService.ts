...
export class ServerAuthService {
  ...
  public async getCurrentSession(): Promise<AuthSession | null> {
    try {
      // First, try to get Clerk session
      ...
      if (clerkUser) {
        const authUser: AuthUser = {
          ...
          isAdmin: false, // Clerk users are regular users
      ...
      // If no Clerk user, check for admin session in cookies
      // Note: Admin sessions are typically handled client-side
      // This is a fallback for server-side admin checks
      return null;
    } catch (error) {
  ...
  // Get Clerk user directly (for server-side operations)
  public async getClerkUser() {
    return await currentUser();
  }

  // Get Clerk auth object (for server-side operations)
  public getClerkAuth() {
    return auth();
  }
}
