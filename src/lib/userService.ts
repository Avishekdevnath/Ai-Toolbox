...
import { determineProvider, getAllProviders } from './utils/providerUtils';
...
export class UserService {
   ...
   */
  async syncUserFromClerk(clerkUser: User): Promise<UserModel> {
    try {
      ...
      const actualProvider = determineProvider(clerkUser);
      const allProviders = getAllProviders(clerkUser);
      ...
      if (clerkUser.externalAccounts && clerkUser.externalAccounts.length > 0) {
        clerkUser.externalAccounts.forEach(account => {
          ...
          oauthAccounts.push({
            provider,
            providerAccountId: account.providerUserId || account.id,
   ...
   */
  async incrementUserStats(clerkId: string, statType: keyof UserModel['stats']): Promise<void> {
    try {
      const db = await getDatabase();
      const collection = db.collection(this.collectionName);
      ...
      console.log(`✅ User stats incremented: ${clerkId} - ${statType}`);
    } catch (error: any) {
   ...
   */
  async incrementToolUsage(clerkId: string, toolName: string): Promise<void> {
    try {
      const db = await getDatabase();
      const collection = db.collection(this.collectionName);
      ...
      console.log(`✅ Tool usage incremented: ${clerkId} - ${toolName}`);
    } catch (error: any) {
   ...
   * Get user statistics
   */
  async getUserStats(clerkId: string): Promise<UserModel['stats'] | null> {
    try {
      const db = await getDatabase();
      const collection = db.collection(this.collectionName);
