...
import AIReport from '@/components/forms/ai-report/AIReport';

export default async function FormAIReportPage({
  params,
}: {
  ...
  const form = await Form.findById(id)
    ...
    .lean<{ title: string; aiInsights?: { data?: { report?: any } } }>();
  if (!form) notFound();

  const subNav = [
    { label: 'Edit',      href: `/dashboard/forms/${id}/edit`,      icon: Edit3 },
    { label: 'Responses', href: `/dashboard/forms/${id}/responses`, icon: MessageSquare },
    { label: 'Analytics', href: `/dashboard/forms/${id}/analytics`, icon: BarChart },
    { label: 'AI Report', href: `/dashboard/forms/${id}/ai-report`, icon: Sparkles, active: true },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <AIReport
        formId={id}
        cachedReport={form.aiInsights?.data?.report}
      />
    </div>
  );
}
