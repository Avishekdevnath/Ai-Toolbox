...
export default function UserCreateModal({ isOpen, onClose, onCreate }: Props) {
  ...
  function renderUsernameIcon() {
    if (usernameStatus === 'checking') return <Loader2 className="w-3.5 h-3.5 animate-spin text-slate-400" />;
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size="xl" className="max-w-[72rem] rounded-2xl border-slate-200/80">
      <ModalHeader className="flex items-center justify-between px-7 py-5 border-b border-slate-200 bg-gradient-to-r from-slate-50 via-white to-blue-50/60">
        <div className="flex items-start gap-3">
          <div className="mt-0.5 flex h-8 w-8 items-center justify-center rounded-lg bg-blue-100 text-blue-700">
            <Plus className="w-4 h-4" />
          </div>
          <div>
            <h2 className="text-[18px] font-semibold text-slate-800">Create New User</h2>
            <p className="mt-0.5 text-[12px] text-slate-500">Add profile details, role access, and recovery questions in one flow.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-12">
            <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm xl:col-span-7">
              <div className="flex items-center justify-between">
                <h3 className="text-[12px] font-semibold uppercase tracking-wide text-slate-500">User details</h3>
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-500">Required fields</span>
              </div>
              ...
              <Field label="Username *">
                ...
                {usernameError ? (
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">Letters, numbers, underscores only</p>
                )}
              </Field>
            ...
            <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm xl:col-span-5">
              <div>
              ...
              <div className="max-h-[23rem] space-y-3 overflow-y-auto pr-1">
                {securityQuestions.map((sq, index) => (
                  <div key={index} className="border border-slate-200 rounded-lg p-3 space-y-2 bg-slate-50/40">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-medium text-slate-600">Question {index + 1}</span>
        ...
        <ModalFooter className="flex justify-end gap-2.5 px-7 py-4 border-t border-slate-200 bg-white">
