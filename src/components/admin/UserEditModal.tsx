  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <ModalHeader className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
        <h2 className="text-[15px] font-semibold text-slate-800">Edit User</h2>
      ...
      <form onSubmit={handleSubmit}>
        <ModalContent className="px-6 py-5 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`ml-auto flex-shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium ${statusColor[user.status] ?? 'bg-slate-100 text-slate-500'}`}>
              {roleIcon(user.role)}
        ...
        <ModalFooter className="flex justify-end gap-2.5 px-6 py-4 border-t border-slate-100">
          <button type="button" onClick={onClose} disabled={loading} className="h-9 px-4 text-[13px] font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50">
            Cancel
          </button>
...
function ActivityStat({ value, label, highlight }: { value: number | string; label: string; highlight?: boolean }) {
  return (
    <div className="bg-slate-50 border border-slate-100 rounded-xl p-3 text-center">
      <p className={`text-[18px] font-bold tabular-nums leading-none ${highlight ? 'text-green-600' : 'text-slate-700'}`}>{value}</p>
      <p className="text-[11px] text-slate-400 mt-1 leading-none">{label}</p>
    </div>
  );
}
