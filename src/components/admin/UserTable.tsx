  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
        ...
        <span className="text-[13px] font-semibold text-slate-800">All Users</span>
        <span className="ml-1 bg-slate-100 text-slate-500 text-[11px] font-medium rounded-full px-2 py-0.5">{users.length}</span>
      </div>

      {users.length === 0 ? (
        <div className="py-16 text-center">
          <Users className="w-10 h-10 text-slate-300 mx-auto mb-3" />
          <p className="text-[13px] font-medium text-slate-600 mb-1">No users found</p>
          <p className="text-[12px] text-slate-400 mb-4">Get started by creating your first user</p>
        ...
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-[13px]">
              ...
              <tbody>
                {users.map((u, i) => (
                  <tr key={`${u._id}-${i}`} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-slate-500">
                      ...
                      {u.activity?.lastActivity && <div className="text-[11px] text-slate-400">Last: {fmt(u.activity.lastActivity)}</div>}
                    </td>
                    <td className="py-3 px-4 text-slate-400">{fmt(u.createdAt)}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1">
                        <button onClick={() => onView(u)} title="View" className="p-1.5 rounded hover:bg-slate-100 text-slate-500 transition-colors"><Eye className="w-3.5 h-3.5" /></button>
                        <button onClick={() => onEdit(u)} title="Edit" className="p-1.5 rounded hover:bg-slate-100 text-slate-500 transition-colors"><Edit className="w-3.5 h-3.5" /></button>
                        {u.status === 'active'
          ...
          {pagination && (
            <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => onPageChange(currentPage - 1)} disabled={!pagination.hasPrevPage} className="p-1.5 rounded border border-slate-200 hover:bg-slate-50 disabled:opacity-40 transition-colors">
                  <ChevronLeft className="w-3.5 h-3.5 text-slate-500" />
                </button>
                <span className="text-[12px] text-slate-500">Page {pagination.currentPage} of {pagination.totalPages}</span>
                <button onClick={() => onPageChange(currentPage + 1)} disabled={!pagination.hasNextPage} className="p-1.5 rounded border border-slate-200 hover:bg-slate-50 disabled:opacity-40 transition-colors">
