  ...
  return (
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2"><MessageSquare className="h-4 w-4 text-[var(--color-text-muted)]" /><h2 className="text-[13px] font-semibold text-[var(--color-text-primary)]">User Feedback</h2></div>
        <Link href="/admin/feedback" className="text-[12px] text-blue-600 hover:underline">View all</Link>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total', value: stats.total, icon: MessageSquare, color: 'text-[var(--color-text-secondary)]' },
...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-center"><Loader2 className="h-7 w-7 animate-spin mx-auto mb-3 text-blue-600" /><p className="text-[13px] text-[var(--color-text-muted)]">Loading dashboard...</p></div>
    </div>
  );
  if (error) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-center"><AlertCircle className="h-7 w-7 mx-auto mb-3 text-red-500" /><p className="text-[13px] text-red-600 mb-3">{error}</p>
        <button onClick={refreshData} className="inline-flex items-center gap-2 px-3 py-1.5 border border-[var(--color-border)] rounded-lg text-[13px] text-[var(--color-text-secondary)] hover:bg-[var(--color-muted)]"><RefreshCw className="h-3.5 w-3.5" />Retry</button>
      </div>
    </div>
  );
  if (!stats) return null;
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-[var(--color-text-primary)]">System Overview</h1>
          <p className="text-[12px] text-[var(--color-text-muted)] mt-0.5">Last updated: {new Date(stats.lastUpdated).toLocaleTimeString()}</p>
        </div>
        <button onClick={refreshData} disabled={refreshing} className="inline-flex items-center gap-2 px-3 py-1.5 border border-[var(--color-border)] bg-[var(--color-surface)] rounded-lg text-[13px] text-[var(--color-text-secondary)] hover:bg-[var(--color-muted)] disabled:opacity-50">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(({ label, value, sub, icon: Icon }) => (
          <div key={label} className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-4 relative">
            ...
            <p className="text-[11px] uppercase tracking-wide text-[var(--color-text-muted)] font-medium">{label}</p>
            <p className="text-2xl font-bold tabular-nums text-[var(--color-text-primary)] mt-1">{value}</p>
            <p className="text-[12px] text-[var(--color-text-muted)] mt-0.5">{sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4"><Server className="h-4 w-4 text-[var(--color-text-muted)]" /><h2 className="text-[13px] font-semibold text-[var(--color-text-primary)]">System Health</h2></div>
          <div className="grid grid-cols-2 gap-y-3 gap-x-6">
            {[
              ...
              { label: 'Last Downtime', value: stats.systemHealth.lastDowntime, color: 'text-[var(--color-text-muted)]' },
          ...
          <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
            <Link href="/admin/system" className="flex items-center justify-center gap-2 w-full py-1.5 border border-[var(--color-border)] rounded-lg text-[13px] text-[var(--color-text-secondary)] hover:bg-[var(--color-muted)]"><Settings className="h-3.5 w-3.5" />System Settings</Link>
          </div>
        </div>

        <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4"><AlertCircle className="h-4 w-4 text-[var(--color-text-muted)]" /><h2 className="text-[13px] font-semibold text-[var(--color-text-primary)]">Recent Alerts</h2></div>
          <div className="space-y-2">
            {stats.alerts.map((alert, i) => (
              <div key={i} className={`flex items-start gap-2.5 p-2.5 rounded-lg border ${alertStyles[alert.type] ?? alertStyles.info}`}>
                ...
                <div className="flex-1 min-w-0"><p className="text-[13px] font-medium text-[var(--color-text-primary)] truncate">{alert.message}</p><p className="text-[11px] text-[var(--color-text-muted)]">{alert.time}</p></div>
              </div>
            ))}
          </div>
          <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
            <Link href="/admin/alerts" className="flex items-center justify-center gap-2 w-full py-1.5 border border-[var(--color-border)] rounded-lg text-[13px] text-[var(--color-text-secondary)] hover:bg-[var(--color-muted)]"><Eye className="h-3.5 w-3.5" />View All Alerts</Link>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4"><BarChart2 className="h-4 w-4 text-[var(--color-text-muted)]" /><h2 className="text-[13px] font-semibold text-[var(--color-text-primary)]">Tool Usage — Last 24 Hours</h2></div>
        {stats.toolUsage.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {stats.toolUsage.map((tool) => (
              <div key={tool.name} className="border border-[var(--color-border)] rounded-lg p-3 hover:bg-[var(--color-muted)]">
                <p className="text-[13px] font-medium text-[var(--color-text-secondary)] truncate">{tool.name}</p>
                <p className="text-xl font-bold tabular-nums text-[var(--color-text-primary)] mt-1">{tool.count}</p>
        ...
        ) : <p className="text-[13px] text-[var(--color-text-muted)] text-center py-8">No tool usage data available</p>}
      </div>

      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2"><Activity className="h-4 w-4 text-[var(--color-text-muted)]" /><h2 className="text-[13px] font-semibold text-[var(--color-text-primary)]">Recent Activity</h2></div>
        </div>
          ...
          {stats.recentActivity.map((item, i) => (
            <div key={i} className="flex items-center justify-between py-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <div><p className="text-[13px] font-medium text-[var(--color-text-secondary)]">{item.user}</p><p className="text-[12px] text-[var(--color-text-muted)]">{item.action}</p></div>
              </div>
        ...
        <div className="mt-3 pt-3 border-t border-[var(--color-border)]">
          <Link href="/admin/activity" className="flex items-center justify-center gap-2 w-full py-1.5 border border-[var(--color-border)] rounded-lg text-[13px] text-[var(--color-text-secondary)] hover:bg-[var(--color-muted)]"><Activity className="h-3.5 w-3.5" />View All Activity</Link>
        </div>
      </div>

      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-5">
        <h2 className="text-[13px] font-semibold text-[var(--color-text-primary)] mb-4">Quick Actions</h2>
        <div className="grid grid-cols-3 gap-3">
          {[{ href: '/admin/users', icon: Users, label: 'Manage Users' }, { href: '/admin/tools', icon: Settings, label: 'Manage Tools' }, { href: '/admin/analytics', icon: BarChart2, label: 'View Analytics' }].map(({ href, icon: Icon, label }) => (
            <Link key={href} href={href} className="flex flex-col items-center gap-2 py-4 border border-[var(--color-border)] rounded-lg hover:bg-[var(--color-muted)] text-[13px] text-[var(--color-text-secondary)]">
